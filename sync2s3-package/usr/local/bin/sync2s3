...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="This script mirror new items from local only to remote S3 endpoint"
    )
    parser.add_argument(
        "-l", "--location", help="local location", default=default_location
    )
    parser.add_argument("-b", "--bucket", default=default_bucket)
    parser.add_argument("-e", "--endpoint", default=default_s3_endpoint)

    parser.add_argument(
        "-f", "--force", action="store_true", help="Do not interactive, no confirmation"
    )
    ...
    for root, dirs, files in os.walk(args.location):
        for file in files:
            ...
            if relative_path in list_for_upload:
                local_path = os.path.join(root, file)
                relative_path = os.path.relpath(local_path, args.location)
                if mimetypes.guess_type(relative_path)[0] is None:
                    ...
                    s3.Bucket(args.bucket).upload_file(
                        ...
                        relative_path,
                        ExtraArgs={
                            "ContentType": mimetypes.guess_type(relative_path)[0]
                        },
                    )

    for entry in list_for_deletion:
        s3.Bucket(args.bucket).delete_objects(
            Delete={"Objects": [{"Key": entry}], "Quiet": True}
        )
